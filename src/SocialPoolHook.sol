...
import {ISocialAVS} from "./interfaces/ISocialAVS.sol";

contract SocialPoolHook is BaseHook {
    using PoolIdLibrary for PoolKey;
    using StateLibrary for IPoolManager;

    ISocialAVS public socialPoolAvs;
    ...
    // Events
    event SwapExecuted(
        address indexed sender, uint256 amountIn, uint256 amountOut, int256 sentiment, uint256 volumeChange
    );
    ...
    constructor(IPoolManager _poolManager, ISocialAVS _avs, IERC20 _socialToken, Currency _pairedToken)
    ...
    function getCurrentSentiment() public view returns (int256) {
        ...
        return socialPoolAvs.getSignal(blockFrom, blockTo);
    }
     ...
     */
    function calculateLiquidityAdjustment(int256 interestDelta, int256 volumeTrend)
        internal
        view
        returns (bool shouldAdjust, int256 adjustmentAmount)
    {
     ...
     */
    function getSocialTokenPoolData() public view returns (uint160 sqrtPriceX96, int24 tick, uint128 liquidity) {
     ...
     * Is called by an external contract (SocialPoolPeriphery ?) periodically
     */
    function beforeAddLiquidity(
        ...
        int256 volumeTrend = getVolumeTrend(address(socialToken));
        ...
        require(params.tickLower >= lowerTick && params.tickUpper <= upperTick, "Position outside adjusted tick range");
        ...
        if (params.liquidityDelta > 0) {
            ...
            if (interestDelta > 0 && volumeTrend > 0) {
                ...
                if (baseAdjustment > 0) {
                    IPoolManager.ModifyLiquidityParams memory newParams = params;
                    newParams.liquidityDelta = int256(baseAdjustment);
                    try poolManager.modifyLiquidity(key, newParams, abi.encode(sender)) {
    ...
    function beforeRemoveLiquidity(
        ...
        if (interestDelta < 0 && volumeTrend < 0) {
            ...
            IPoolManager.ModifyLiquidityParams memory newParams = params;
            newParams.liquidityDelta = int256(baseAdjustment);
            try poolManager.modifyLiquidity(key, newParams, abi.encode(sender)) returns (
